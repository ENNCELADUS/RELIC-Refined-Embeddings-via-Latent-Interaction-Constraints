...
- Success criteria for the *next* commit: `mypy src tests` passes in `relic`, and pipeline modes have explicit integration coverage.
...
1. Install and execute mypy in `relic`; fix strict typing errors in `src/model/v3.py` and pipeline modules.
...
4. Add tests for `full_pipeline`, `train_only`, `eval_only`.
